...
// Небольшие измения в прототипов стандартных объектов javascript которых может не быть в IE8

// Дело бесполезное, потому что если верить тому что выдает IE8, DOM элементы произошли  сразу от HTMLDocument:
if(!Element.prototype.addEventListener || !HTMLDocument.prototype.addEventListener){
	// не работает, но и не сообщает об ошибке
	Element.prototype.addEventListener=function(eventName, handler){
		this.attachEvent("on"+eventName,handler);
	};
	// На document работает
	HTMLDocument.prototype.addEventListener=function(eventName, handler){
		this.attachEvent("on"+eventName,handler);
	};
}
...
if (!Array.prototype.forEach){
	Array.prototype.forEach=function(handle){
		if (!handle && !handle.call) throw("Undefined function in arguments forEach() method");
		var i=0;
		while(this[i]){
			handle(this[i]);
			i++;
...
// создаем заглушку для объекта console
if(!window.console){
	window.console={
		log:function(){},
		dir:function(){}
